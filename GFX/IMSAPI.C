...
)
{
   ...
   /* port: game code busy-waits on this via "while (IMS_CheckAck());" /
      "while (IMS_IsAck());" all over WINDOWS.C/RAP.C/STORE.C/INTRO.C with
      no FRAME_COUNT in sight, so - unlike those loops - this is the only
      choke point where such a wait can pump SDL events and let the
      awaited button/key state actually change. Without it these loops
      spin forever on stale state. */
   PLAT_Pump();
   PLAT_MicroSleep();
...
)
{
   ...
   PLAT_Pump();          /* see IMS_CheckAck for why this is load-bearing */
   PLAT_MicroSleep();
