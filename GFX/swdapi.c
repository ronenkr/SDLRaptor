...
/* port: see the comment on SFIELD::sptr_reserved in SWDAPI.H - the real
   "saved picture" pointer for a field lives here, keyed by the field's
   own (GLB-locked, stable-while-open) address. */
#define MAX_FIELD_SPTRS 256
typedef struct { SFIELD *field; BYTE *ptr; } FIELD_SPTR_SLOT;
PRIVATE FIELD_SPTR_SLOT field_sptrs[ MAX_FIELD_SPTRS ];
...
)
{
   ...
   for ( i = 0; i < MAX_FIELD_SPTRS; i++ )
      if ( field_sptrs[i].field == fld )
         return field_sptrs[i].ptr;
...
)
{
   ...
   for ( i = 0; i < MAX_FIELD_SPTRS; i++ )
   {
      if ( field_sptrs[i].field == fld )
      {
         field_sptrs[i].ptr = ptr;
         if ( ptr == NUL )
            field_sptrs[i].field = NUL;
         return;
      }
      if ( free_slot < 0 && field_sptrs[i].field == NUL )
         free_slot = i;
   }
   ...
   if ( free_slot < 0 )
      EXIT_Error ("SWD_SetFieldSptr: out of slots");

   field_sptrs[ free_slot ].field = fld;
   field_sptrs[ free_slot ].ptr   = ptr;
}
...
)
{
   ...
   {
      BYTE *fld_sptr = SWD_GetFieldSptr ( curfld );

      if ( curfld->saveflag && fld_sptr )
         GFX_PutImage ( fld_sptr, fld_x, fld_y, FALSE );
   }
...
)
{
   ...
   for ( loop = 0; loop < header->numflds; loop++, fld++, numflds++ )
   {
      if ( fld->opt != FLD_OFF )
      {
         ...
         if ( fld->saveflag && SWD_GetFieldSptr ( fld ) )
         {
            picdata = SWD_GetFieldSptr ( fld );
         ...
         for ( loop = 0; loop < header->numflds ; loop++ , curfld++ )
         {
            if ( curfld->opt != FLD_OFF )
            {
               ...
               if ( curfld->saveflag )
               {
                  ...
                  new_sptr = (BYTE *) malloc ( pic_size );
                  if ( !new_sptr )
                     EXIT_Error ("SWD Error: out of memory");
                  SWD_SetFieldSptr ( curfld, new_sptr );
               }
            }
         }
...
)
{
   ...
   for ( loop = 0; loop < curwin->numflds ; loop++, curfld++ )
   {
      ...
      if ( curfld->saveflag && SWD_GetFieldSptr ( curfld ) != NUL )
      {
         free ( SWD_GetFieldSptr ( curfld ) );
         SWD_SetFieldSptr ( curfld, NUL );
      }
   }
