...
#include "tsmapi.h"
#include "dpmiapi.h"
#include "plat.h"        /* Win32/SDL3 port: window + palette + present */
...
)
{
   ...
   /* On VGA this waited for vertical retrace; that wait paced the
      palette-fade loops, so keep a 70 Hz gate in its place. */
   if ( retraceflag )
      PLAT_WaitNextTick();

   PLAT_GetPalette ( curpal );
   memcpy ( curpal + start_pal * 3, palette + start_pal * 3,
            ( 256 - start_pal ) * 3 );
   PLAT_SetPalette ( curpal );
   PLAT_Present();
}
...
{
   CHAR * err = "GFX_Init() - memory";
   INT   loop;

   /* displaybuffer/displayscreen are plain buffers now; the extra 256
      bytes of slack absorb the cursor blitters' deliberate overreads at
      the screen edges (harmless on real VGA, fatal on a tight heap).
      The 256-alignment the DOS build needed for the shade tables was an
      ASM indexing trick; the C blitters index normally. */
   displaybuffer = ( BYTE * ) calloc ( 64000 + 256, 1 );
   displayscreen = ( BYTE * ) calloc ( 64000 + 256, 1 );
   ltable        = ( BYTE * ) calloc ( 256, 1 );
   dtable        = ( BYTE * ) calloc ( 256, 1 );
   gtable        = ( BYTE * ) calloc ( 256, 1 );

   if ( !displaybuffer || !displayscreen || !ltable || !dtable || !gtable )
      EXIT_Error(err);

	tsm_id = TSM_NewService( (void (*)(void))GFX_TimeFrameRate, 70, 255, 0 );
